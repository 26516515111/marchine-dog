...
### 1. 类别策略创新：单类 fire 检测 + firebig 后处理

传统做法是直接训练评测要求的类别，但本项目发现 firebig 是评测定义的提交格式而非物理可分的对象类别。核心创新在于放弃在模型层面区分 fire/firebig，而是统一检测 fire，在推理后处理中通过可信度筛选和面积最大策略动态输出 firebig。这样做既保留了所有火焰样本的完整学习信号，又满足了提交格式要求。

### 2. 领域随机化增强策略

受 Domain Randomization 和 ImageNet-C 启发，针对火焰检测场景设计了一套**适度退化增强组合**：GaussianBlur（模拟对焦模糊）、MotionBlur（模拟运动抖动）、GaussianNoise（模拟传感器噪声）、RandomErasing（模拟局部遮挡和光斑），每个增强的概率控制在 0.08 的低水平，在不破坏火焰主要特征的前提下提升模型对退化场景的鲁棒性。

### 3. 混杂数据集构建方法论

提出**1:2 混杂比例**的通用原则：纯净数据（1 份）保证标注质量和主分布稳定，混杂数据（2 份）引入复杂光照、烟雾氛围等环境变化。该比例的核心思路是让混杂数据作为补充而非主导，避免模型被复杂样本过度牵引。

### 4. 动态阈值后处理机制

predict.py 中实现了**绝对阈值 + 相对最高分**的双重动态筛选：
- 绝对阈值 0.1 保证基础召回
- 相对最高分比例 0.5 动态过滤弱候选
- 当模型对某张图整体置信度高时自动收紧，置信度低时自动放宽
- 每张图自适应调整，比静态阈值更贴近实际置信度分布

### 5. 结构化错误决策记录

项目系统性地记录了三个关键错误决定（类别策略、数据集选择、模型选择），每个都包含错误决策、问题分析和后期改正三部分。这种结构化反思不仅帮助避免了重复踩坑，还为后续项目提供了可复用的决策经验。
