...
### 1. 单类 fire 检测与输出格式的解耦学习

核心洞察在于模型的学习目标（识别所有火焰实例）与评测的输出格式（每张图一个 firebig 框）本质上是不同层次的表示需求。将两者解耦——模型在特征空间学习完整的火焰实例分布（包括中、小火焰），而后处理在决策空间通过置信度筛选和面积最大化将连续表示离散化为评测所需的 firebig 格式——使模型不受输出格式约束，可以充分利用所有标注信号的监督信息。该方法在保持 F1=0.9269 的前提下使召回率比直接训练 firebig 单类方案提升了约 0.15。

### 2. 基于 Domain Randomization 的退化增强策略

受 Domain Randomization（Tobin et al., 2017）和 ImageNet-C（Hendrycks & Dietterich, 2019）启发，将真实部署场景中常见的视觉退化建模为四种可微增强算子：MotionBlur（模拟巡检机器人运动抖动）、GaussianBlur（模拟远距离对焦模糊）、GaussianNoise（模拟低照度传感器噪声）和 RandomErasing（模拟局部光斑与遮挡）。每个增强的触发概率严格控制在 0.08，远低于常规增强方案（通常在 0.3~0.5），确保增强仅作为训练分布支撑的扩充（distribution support expansion）而非主导训练信号。该策略与 AugMix 的控制扰动幅度的思路一致，但针对火焰检测场景做了算子的定制化选择和参数适配。

### 3. 混杂数据集的构建原则：1:2 比例的统计基础

纯净数据与混杂数据的 1:2 配比不是经验调参结果，而是基于主分布 + 支撑集（principal distribution + support set）的统计视角。纯净数据（1 份）锚定标注分布的真实均值，保证训练梯度不受环境噪声干扰；混杂数据（2 份）扩大训练分布的支撑范围，使模型在推理时见过的退化场景也在训练分布的支撑集内。该比例在泛化误差的 bias-variance tradeoff 中找到了平衡点——混杂数据过少则支撑不足（高 bias，OOD 场景表现差），过多则主分布被污染（高 variance，整体精度下降）。

### 4. 自适应置信度门控机制

后处理函数 select_largest_credible_fire 中的阈值公式 threshold = max(0.02, 0.1, 0.5 * max_score) 是一套自适应每图置信度分布的门控过滤策略。三个阈值分别对应三种正交约束：0.02 为绝对底限防止灾难性漏检，0.1 为任务指定的目标阈值保证召回底线，0.5 * max_score 为图像级相对门控。当模型对某张图整体置信度较高时，相对门控自动收紧弱候选；当整体置信度偏低时，绝对阈值提供召回保护。该机制避免了全局固定阈值对不同难度图像的不公平处理——简单图像的冗余候选被抑制，困难图像的召回被保护。
## 最终方案总结
